...
interface ChatSettingsData {
  ...
  agents: {
    primary_agent: number;
    child_agents_enabled: boolean;
...
export const ChatSettings: React.FC<ChatSettingsProps> = ({
  ...
  const [settings, setSettings] = useState<ChatSettingsData>({
    ...
    agents: {
      primary_agent: (selectedAgent as { id?: number })?.id || 1,
  ...
  const handleSettingChange = (category: keyof ChatSettingsData, key: string, value: unknown) => {
    setSettings(prev => ({
      ...prev,
      [category]: {
        ...prev[category],
