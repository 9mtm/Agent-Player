...
export const storageSearchTool = {
    name: 'storage_search',
    description: 'Search files stored in local storage (cache and CDN). Find previously saved images, audio, avatars, documents, and other files.',
    input_schema: {
        type: 'object',
        properties: {
            query: {
    ...
    async execute(params) {
        try {
            const { query, zone, category, tags: tagsStr, limit = 10, } = params;
