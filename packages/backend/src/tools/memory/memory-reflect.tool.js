...
export const memoryReflectTool = {
    name: 'memory_reflect',
...
- focus: Optional topic to focus the reflection on (e.g. "coding preferences")`,
    ...
    async execute(params) {
        const { userId, lookbackCount = 20, focus } = params;
        ...
        try {
            ...
            const targetMemories = focus
                ? recentMemories.filter(m => m.content.toLowerCase().includes(focus.toLowerCase()) ||
                    (m.metadata?.tags || []).some((t) => t.toLowerCase().includes(focus.toLowerCase())))
                : recentMemories;
