...
- Associated metadata`,
    ...
    async execute(params) {
        try {
            ...
            // Search memories
            const memories = await storage.search(params.query, params.userId, params.limit || 5);
            // Apply filters
            ...
            if (filtered.length === 0) {
                return {
                    content: [
                        {
                            type: 'text',
                            text: `No memories found for: "${params.query}"${params.type ? `\nType filter: ${params.type}` : ''}${params.minImportance ? `\nMinimum importance: ${params.minImportance}` : ''}${params.tags ? `\nTags filter: ${params.tags.join(', ')}` : ''}\n\nTry:\n- Using a broader search query\n- Removing filters\n- Checking if this information was previously saved`,
                        },
                    ],
                    data: {
                        success: true,
            ...
            if (params.type || params.minImportance || params.tags) {
                ...
                if (params.type)
                    responseText += `- Type: ${params.type}\n`;
                if (params.minImportance)
                    responseText += `- Min importance: ${params.minImportance}\n`;
                if (params.tags)
                    responseText += `- Tags: ${params.tags.join(', ')}\n`;
            }
            return {
                content: [
                    {
