...
export const claudeCliTool = {
    name: 'claude_cli',
    description: 'Send a prompt to the locally-installed Claude Code CLI and get the response. ' +
    ...
    input_schema: {
        ...
        properties: {
            ...
            workdir: {
                type: 'string',
                description: 'Working directory for the Claude Code process (optional). ' +
                    'Defaults to the current backend working directory.',
            },
            model: {
                type: 'string',
                description: 'Model to use (optional). Example: "claude-sonnet-4-5-20250929". ' +
    ...
    async execute(params) {
        ...
        if (workdir)
            console.log(`[ClaudeCLI]   Workdir: ${workdir}`);
        if (model)
            console.log(`[ClaudeCLI]   Model: ${model}`);
        // Resolve API key: env var → first active Claude agent in DB
        let anthropicApiKey = process.env.ANTHROPIC_API_KEY || '';
        if (!anthropicApiKey) {
            try {
                const db = getDatabase();
                const row = db.prepare(`SELECT api_key FROM agents_config WHERE provider = 'claude' AND api_key IS NOT NULL AND api_key != '' LIMIT 1`).get();
                if (row?.api_key)
