...
const SCRIPT_PATH = path.join(process.cwd(), 'python-scripts', 'tools', 'desktop', 'desktop.py');
function runDesktopScript(args) {
...
// ─── Tool definition ──────────────────────────────────────────────────────────
export const desktopControlTool = {
    ...
    async execute(params) {
        ...
        try {
            ...
            const actionLabel = {
                ...
                scroll: `Scrolled ${params.amount > 0 ? 'up' : 'down'} ${Math.abs(params.amount ?? 3)} at (${params.x}, ${params.y})`,
                drag: `Dragged from (${params.start_x}, ${params.start_y}) to (${params.end_x}, ${params.end_y})`,
                key_press: `Key pressed: ${params.keys}`,
                type_text: `Typed ${params.text?.length ?? 0} characters`,
                wait: `Waited ${params.seconds ?? 1} seconds`,
                get_screens: `Found ${result.count ?? '?'} monitor(s)`,
                get_active_window: `Active window: "${result.title ?? ''}" on screen ${result.screen ?? '?'}`,
                show_indicator: `Indicator shown on screen ${result.screen ?? '?'} — Monitor offset: left=${result.monitor?.left ?? 0}, top=${result.monitor?.top ?? 0} — IMPORTANT: all click/scroll coordinates must be ABSOLUTE (add left offset to local X, add top offset to local Y)`,
