...
interface S3Config {
  ...
  publicUrl?: string;  // CloudFront or R2 public URL prefix
...
export class S3StorageProvider implements StorageProvider {
  ...
  constructor(name: 's3' | 'r2', config: S3Config) {
    this.name = name;
    this.config = config;
  }
  ...
  private async getClient(): Promise<any> {
    ...
    try {
      ...
      throw new Error(
        `@aws-sdk/client-s3 is not installed. Run: pnpm add @aws-sdk/client-s3 --filter backend`
      );
    }
  ...
  getReadStream(_key: string): Readable | null {
    return null;
  }
...
export function createR2ProviderFromEnv(): S3StorageProvider {
