...
export class MemoryStorage {
    ...
    `);
        stmt.run(memory.id, memory.userId, memory.type, memory.content, JSON.stringify(memory.embedding), memory.layer || 'daily', memory.status || 'active', JSON.stringify(memory.metadata), memory.importance, memory.tags ? JSON.stringify(memory.tags) : null, memory.createdAt.toISOString(), memory.lastAccessed?.toISOString() || null, memory.accessCount);
